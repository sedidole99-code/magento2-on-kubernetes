...
- [ ] **Enriched Varnish VCL** — upgrade `deploy/bases/varnish/default.vcl` (currently minimal) to production-grade caching logic. Add: **grace mode** to serve stale content for N seconds while revalidating in the background (prevents cache stampedes when a hot object expires under load); **saint mode** to keep serving stale while the backend is unhealthy (gated on `std.healthy(req.backend_hint)`); **tag-based invalidation** honouring Magento's `X-Magento-Tags` PURGE header to evict selectively by product / category / CMS-block ID instead of a full flush; **ESI handling** for per-customer blocks (cart summary, customer greeting, messages); and **bot-aware caching** (treat crawlers as logged-out to avoid polluting the session-variant cache). The current VCL caches aggressively but lacks these escape hatches — a single slow PHP request cascades into upstream pressure without grace mode, and invalidating one product requires a full purge. Must stay compatible with Magento's built-in Varnish integration (which issues `PURGE` with regex-matched `X-Magento-Tags`).
...
- [+] **Database read replicas** — add [ProxySQL](https://proxysql.com/) as a query router between Magento pods and MySQL with read/write splitting enabled. Deploy as a standalone Service (not a sidecar — sidecars force ProxySQL config churn on every pod restart). Routing: `SELECT` queries outside transactions go to replicas (`hostgroup_id=10`), everything else to the primary (`hostgroup_id=0`). Replica topology: either Percona XtraDB Cluster (synchronous, multi-primary) or MySQL native async replicas (simpler, single-primary). ProxySQL state lives in a ConfigMap covering `mysql_servers`, `mysql_users`, `mysql_query_rules`, and `mysql_replication_hostgroups`; configuration changes require a `LOAD ... TO RUNTIME` on the admin interface (port 6032). Ship a reconcile CronJob that re-applies the ConfigMap to ProxySQL's runtime on changes, and a `users-reload` Job that re-syncs `mysql_users` when the MySQL credentials Secret rotates. Land as `deploy/components/proxysql/` — opt-in, since single-instance deployments don't need the complexity and it adds two hops per query. Heavy catalog browsing (category pages, layered navigation, search) generates read-heavy queries that can saturate a single MySQL instance; replicas offload SELECTs while the primary handles writes.
...
- [ ] **Dedicated restore Jobs** — convert `make restore-db` / `make restore-media` from ad-hoc `kubectl exec` shell-outs into proper Kubernetes `Job` manifests under `deploy/bases/backup/` (e.g. `db-restore-job.yaml`, `media-restore-job.yaml`). Current Makefile targets shell into a running pod, which produces no auditable history, can't retry on transient failures, and skips pre-restore safety checks. The Job approach: render the backup filename via `kustomize edit set` or `envsubst`, pre-flight with a size + checksum check against the backup PVC, emit an automatic pre-restore DB dump for reversibility, perform the drop + import under a maintenance-mode scale-down of `magento-web` + `magento-consumer`, and stream progress to stdout so `kubectl logs -f job/db-restore` shows live status. Parameterize via `make restore-db BACKUP_NAME=…` which renders and applies the Job spec. Set `ttlSecondsAfterFinished: 3600` so completed Jobs self-clean. Mirror the same pattern for media restore, with its own pre-check (target PVC free space ≥ archive size × 1.2).
