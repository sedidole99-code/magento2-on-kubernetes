...
| **imgproxy** | None | `deploy/components/imgproxy/` (opt-in; on in production) — routes `/media/catalog/product/` images through imgproxy for in-memory WebP/AVIF transcode (wired via the nginx ConfigMap mount on magento-web). Reads media PVC directly (`IMGPROXY_LOCAL_FILESYSTEM_ROOT=/local`); co-scheduled via `podAffinity` to magento-web (RWO PVC constraint). nginx ConfigMap replaced by component. Signing deferred (ClusterIP + NetworkPolicy boundary). Production HPA: min 2 / max 10. |
...
- [x] **imgproxy for on-the-fly image optimization** — `deploy/components/imgproxy/` (off in dev/staging, on in production). Routes `/media/catalog/product/` images through imgproxy for in-memory WebP/AVIF transcode. Wired via the nginx ConfigMap mount on magento-web — see "imgproxy toggle" below.
...
- The `nginx` ConfigMap in magento-web is **replaced** with a version that proxies `/media/catalog/product/...` through imgproxy. The routing is delivered via the nginx ConfigMap mount on the magento-web container (`default.conf` and `magento2.conf` are mounted as per-file subPath volumeMounts onto the running nginx — see CLAUDE.md `### nginx ConfigMap mount` for the architecture). nginx strips the `cache/<hash>/` segment from Magento's pre-resize URLs so imgproxy reads the original file on disk. On imgproxy 5xx, nginx falls back to `get.php` (Magento's existing media-fallback path), so users see the original image rather than a 502.
