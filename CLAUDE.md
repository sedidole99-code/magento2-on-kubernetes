...
**Component patch invariant (second face of the drift-watch):** The imgproxy component's `configMapGenerator: behavior: replace` requires BOTH `nginx-default.conf` and `nginx-magento2.conf` to be re-declared in `deploy/components/imgproxy/patches/`. Both patch files must track their base counterparts. A PR adding a new nginx location to the base must also update the component patch files in the same commit — forgetting `nginx-default.conf` causes CrashLoop on `nginx -t` (broken include); forgetting `nginx-magento2.conf` causes imgproxy-enabled deployments to run a stale server block and silently lose any base-side fixes.
