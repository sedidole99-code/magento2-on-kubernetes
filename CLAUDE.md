...
### nginx ConfigMap mount (wires imgproxy routing + unlocks per-env nginx tuning)

The `nginx` ConfigMap (generated by `deploy/bases/app/kustomization.yaml`'s `configMapGenerator`) is mounted on the `magento-web` container via two per-file `subPath` volumeMounts: `default.conf` → `/etc/nginx/conf.d/default.conf` and `magento2.conf` → `/etc/nginx/sites-available/default`. This makes all Kustomize-level nginx edits — both base edits and component `configMapGenerator` patches — reach the running pod.

**Per-file `subPath` is mandatory, not stylistic.** `src/etc/nginx/conf.d/` contains both `default.conf` (upstream fastcgi_backend block, overridden by the mount) and `stub_status.conf` (175 bytes — the nginx-metrics-exporter sidecar's `:6081/stub_status` Prometheus scrape endpoint that feeds the Magento Nginx Grafana dashboard). A directory mount of `/etc/nginx/conf.d/` would replace the directory entirely, clobbering `stub_status.conf` and silently breaking the metrics scrape. Per-file subPath overlays only the targeted file and leaves siblings intact. The same logic applies to the `magento2.conf` mount at `sites-available/default`.

**Mount at the symlink target, not the symlink.** `/etc/nginx/sites-enabled/default` is a symlink to `sites-available/default`. The volumeMount is at the target so the symlink resolves transparently. Mounting at the symlink itself would replace it with a regular file and could break future image updates that expect the symlink topology.

**Source-sync drift-watch invariant (CRITICAL):**
- The canonical source is `src/etc/nginx/...` (image-baked, known-good, running in production for years).
- `deploy/bases/app/config/nginx/conf.d/default.conf` mirrors `src/etc/nginx/conf.d/default.conf` byte-for-byte.
- `deploy/bases/app/config/nginx/magento2.conf` mirrors `src/etc/nginx/sites-available/default` byte-for-byte.
- **A PR that edits EITHER must edit the other in the same commit.** Pre-fix, the ConfigMap was unused so drift went undetected. Post-fix, the cluster runs the ConfigMap version (the image-baked file is silently overlaid by the subPath mount) — drift causes image-vs-cluster divergence: the image looks correct but the cluster runs the drifted version. Debugging this is painful.

**Component patch invariant (second face of the drift-watch):** The imgproxy component's `configMapGenerator: behavior: merge` overrides only the `magento2.conf` key — `default.conf` is inherited from the base unchanged. The component therefore ships exactly one nginx patch file (`deploy/components/imgproxy/patches/nginx-magento2.conf`), which must track its base counterpart `deploy/bases/app/config/nginx/magento2.conf` byte-for-byte (modulo the imgproxy location blocks). A PR adding a new nginx location to the base must also update the component patch in the same commit, or imgproxy-enabled deployments will silently run a stale server block and lose any base-side fixes. (Earlier drafts of this component used `behavior: replace` and shipped a second `nginx-default.conf` patch; that approach was a footgun — forgetting to re-sync `nginx-default.conf` after a base change CrashLooped magento-web on every imgproxy-enabled rollout. `merge` is the correct semantic and is now used.)

**Cut-over caveat:** The first deploy of this fix triggers a magento-web rolling update because the volume list changes → pod-template hash changes. Under the RWO `media` PVC, the surge replica may stall on a different node — use `make deploy-maintenance prod` for this one-shot cutover. Subsequent ConfigMap content edits (changing nginx config, adding a location block) also trigger a pod-template hash change via the configMapGenerator hash suffix, but the volume topology is static. Prefer `make deploy-maintenance prod` as the safe default for any nginx config edit on production.

### imgproxy on-the-fly image optimization (toggle via `deploy/components/imgproxy/`)

Off by default in dev and staging; **on in `deploy/overlays/production`**. Enabling the component adds an `imgproxy` Deployment (image: `darthsim/imgproxy:v3.27`), ClusterIP Service on port 8080, NetworkPolicy, PDB (`minAvailable: 1`), and HPA (base 1–5 replicas, CPU 75%). It also replaces the `nginx` ConfigMap in magento-web with a version that proxies `/media/catalog/product/...` requests through imgproxy for in-memory WebP/AVIF transcode (`IMGPROXY_AUTO_WEBP=true`, `IMGPROXY_AUTO_AVIF=true`, `IMGPROXY_QUALITY=85`). Routes not in that path (`/media/wysiwyg/`, `/static/`, theme assets, favicon) continue serving directly via `try_files`. The routing is delivered via the nginx ConfigMap mount on magento-web (see `### nginx ConfigMap mount` subsection above); the imgproxy location blocks live in `deploy/components/imgproxy/patches/nginx-magento2.conf` and reach nginx only when both the ConfigMap is mounted AND the imgproxy component is enabled.
